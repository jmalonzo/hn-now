...
class HackerNewsClient {
    ...
    async fetchStories(type) {
        ...
        try {
            ...
            return storyData
                .filter((story) => story !== null && story.title !== undefined)
    ...
    formatStoryForDisplay(story, index) {
    ...
    async showStoryMenu(stories) {
        const choices = stories.map((story, index) => ({
        ...
        await this.showStoryActions(selectedStory);
    }
    async showStoryActions(story) {
        const actions = [];
        if (story.url && story.url !== story.commentsUrl) {
            actions.push({
    ...
    async showMainMenu() {
        ...
        switch (choice) {
            ...
            case 'job':
                try {
                    const stories = await this.fetchStories(choice);
